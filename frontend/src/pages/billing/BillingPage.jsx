...
export default function BillingPage() {
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50 dark:bg-gray-955 overflow-hidden font-sans">
      {/* Bill Success Modal */}
      ...
      {/* Header */}
      <header className="h-16 bg-white dark:bg-gray-900 px-8 flex items-center justify-between shrink-0 border-b border-gray-100 dark:border-gray-800 shadow-sm">
        <div>
          <h1 className="text-xl font-bold text-gray-900 dark:text-white">Billing (POS)</h1>
        </div>
        <div className="flex items-center gap-4">
          ...
          <button className="relative p-2 text-gray-400 hover:text-gray-600 dark:text-gray-400 transition-colors">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="text-left leading-none">
              <p className="text-sm font-bold text-gray-800 dark:text-gray-200 capitalize">{user?.name || 'Weiyz'}</p>
      ...
      {/* Main Grid Content */}
      <div className="flex-1 grid grid-cols-[1.3fr_1.2fr_300px] gap-6 p-6 h-[calc(100vh-64px)] overflow-hidden bg-gray-50 dark:bg-gray-955">
        
        {/* Column 1: Billing Inputs Card */}
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm p-6 flex flex-col h-full overflow-y-auto space-y-6">
          {/* Product Search */}
          <div className="relative" ref={searchRef}>
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-4 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-950 text-gray-900 dark:text-white focus:bg-white dark:focus:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 text-sm font-medium shadow-sm transition-all"
            />
            {/* Search results dropdown */}
            {showResults && searchQuery.length >= 2 && (
              <div className="absolute top-full left-0 right-0 z-50 mt-2 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-xl max-h-[40vh] overflow-y-auto">
                {loadingMedicines ? (
                  ...
                  searchResults.map((medicine) => (
                    <button
                      ...
                      className={cn(
                        "w-full flex items-center justify-between p-3.5 transition-colors border-b border-gray-55 last:border-0 text-left",
                    ...
                    >
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-[10px] text-gray-500 dark:text-gray-400">Stock: {medicine.availableQty}</p>
                      </div>
                    </button>
                  ))
                )}
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3.5">
              ...
              <div className="flex items-center justify-between gap-4">
                <span className="text-sm text-gray-500 dark:text-gray-400 font-medium whitespace-nowrap">Bill Discount</span>
                <div className="flex items-center gap-2">
                  <select
                    value={discountType}
                    onChange={(e) => setDiscountType(e.target.value)}
                    className="text-xs border border-gray-200 dark:border-gray-700 rounded px-2 py-1 bg-white dark:bg-gray-900 outline-none focus:ring-1 focus:ring-brand-500"
                  >
                  ...
                  <input
                    ...
                    placeholder="0"
                    className="w-16 text-right text-sm border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-white focus:border-brand-500 outline-none pb-0.5"
                    min="0"
                  />
                </div>
              </div>
              ...
              <div className="flex justify-between text-sm">
                <span className="text-gray-500 dark:text-gray-400 font-medium">Taxable Amount</span>
                <span className="font-bold text-gray-900 dark:text-white">
              ...
              <div className="flex justify-between text-sm">
                <span className="text-gray-500 dark:text-gray-400 font-medium">Total GST (12%)</span>
                <span className="font-bold text-gray-900 dark:text-white">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm flex flex-col h-full overflow-hidden">
          {/* Header */}
          <div className="p-4 border-b border-gray-100 dark:border-gray-800 flex items-center gap-3 bg-gray-50 dark:bg-gray-950/50 shrink-0">
            <div className="h-10 w-10 bg-brand-100 text-brand-600 rounded-xl flex items-center justify-center">
              <ShoppingCart className="h-5 w-5" />
            </div>
            <div>
              <h2 className="font-bold text-gray-900 dark:text-white text-base">Transaction Cart</h2>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {cartItems.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-gray-400 py-12">
                <div className="h-20 w-20 bg-gray-50 dark:bg-gray-950 rounded-full flex items-center justify-center mb-4 border border-gray-100 dark:border-gray-800 shadow-inner">
              ...
              cartItems.map((item, index) => (
                <div
                  ...
                  className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-955/50 rounded-xl border border-gray-100 dark:border-gray-800 hover:border-brand-200 transition-all group"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-gray-900 dark:text-white truncate capitalize">{item.medicineName} {item.medicineDosage}</p>
                  ...
                  <div className="flex items-center bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden shrink-0">
                    <button
                      type="button"
                      onClick={() => updateQuantity(index, item.quantity - 1)}
                      className="p-1.5 hover:bg-gray-50 dark:bg-gray-950 text-gray-600 dark:text-gray-400 transition-colors"
                    >
                      <Minus className="h-3 w-3" />
                    </button>
                    <span className="text-xs font-bold text-gray-800 dark:text-gray-200 w-6 text-center">{item.quantity}</span>
                    <button
                      type="button"
                      onClick={() => updateQuantity(index, item.quantity + 1)}
                      className="p-1.5 hover:bg-gray-50 dark:bg-gray-950 text-gray-600 dark:text-gray-400 transition-colors"
                  ...
                  <div className="min-w-[60px] text-right text-sm font-bold text-gray-900 dark:text-white shrink-0">
          ...
          <div className="p-4 border-t border-gray-100 dark:border-gray-800 flex items-center justify-center bg-gray-50 dark:bg-gray-950/50 shrink-0">
            <span className="text-sm font-semibold text-gray-600 dark:text-gray-400">Current Cart Total: {formatCurrency(totals.subtotal - totals.totalDiscount)}</span>
          </div>
          ...
          <div className="p-4 bg-[#1e1b4b] text-white space-y-4 shrink-0">
            ...
            {user?.role === 'OWNER' && totals.netProfit > 0 && (
              <div className="flex justify-between text-[11px] font-medium px-2.5 py-1.5 bg-white dark:bg-gray-900/10 rounded-lg">
            ...
            <div className="flex gap-2 text-xs">
              <button
                onClick={handlePrint}
                disabled={!lastBill}
                className="flex-1 py-2 bg-white dark:bg-gray-900/10 hover:bg-white dark:bg-gray-900/20 disabled:opacity-30 rounded-lg font-semibold flex items-center justify-center gap-1.5 transition-colors"
              >
        ...
        <div className="bg-[#f1f5f9] dark:bg-gray-950 p-6 flex flex-col gap-4 overflow-y-auto rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm h-full">
          <div className="flex flex-col gap-3">
            {[
              ...
              <button
                ...
                className="flex items-center gap-3 p-3 bg-white dark:bg-gray-900 hover:bg-brand-50 border border-gray-200 dark:border-gray-700 hover:border-brand-300 rounded-xl transition-all shadow-sm group text-left"
              >
                <div className={cn("h-10 w-10 rounded-lg flex items-center justify-center text-lg", item.color)}>
                  {item.icon}
                </div>
                <div className="flex-1">
                  <p className="text-sm font-semibold text-gray-800 dark:text-gray-200 group-hover:text-brand-700 capitalize">
                    {item.label}
                  </p>
                </div>
              </button>
