...
export default function BillingPage() {
  ...
  return (
    <div className="flex flex-col lg:flex-row bg-gray-50 dark:bg-gray-950 font-sans -m-2 sm:-m-4 lg:-m-6 min-h-[calc(100vh-4rem)] lg:h-[calc(100vh-4rem)] lg:overflow-hidden">
      {/* Bill Success Modal */}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl max-w-md w-full overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 text-center border-b border-gray-100 dark:border-gray-800">
              ...
              <h3 className="text-xl font-bold text-gray-900 dark:text-white">Sale Completed!</h3>
              <p className="text-gray-500 dark:text-gray-400">Bill #{lastBill.billNumber} generated successfully</p>
            </div>

            <div id="bill-receipt" className="p-6 bg-gray-50 dark:bg-gray-950 text-sm">
              <div className="text-center mb-4">
                <h4 className="font-bold text-lg">{user?.medicalStoreId?.name || 'Medical Store'}</h4>
                <p className="text-xs text-gray-500 dark:text-gray-400">{user?.medicalStoreId?.address || ''}</p>
              </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 lg:overflow-hidden">
        {/* Header */}
        <header className="py-4 lg:h-20 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 px-4 sm:px-8 flex flex-col sm:flex-row items-center justify-between shrink-0 z-30 shadow-sm gap-4 relative overflow-hidden">
          ...
          <div className="flex items-center gap-4 w-full sm:w-auto mt-1 lg:mt-0">
            ...
            <div>
              <h1 className="text-xl font-medium text-gray-900 dark:text-white tracking-tight">Point of Sale</h1>
              <p className="text-xs font-medium text-gray-500 dark:text-gray-400 flex items-center gap-1.5 mt-0.5">
                <span>Bill No:</span>
                <span className="font-mono bg-slate-100 dark:bg-gray-800 text-slate-700 dark:text-gray-300 px-1.5 py-0.5 rounded text-[10px] tracking-wider border border-slate-200 dark:border-gray-700">{billNumber}</span>
              </p>
            </div>
          </div>

          <div className="flex items-center gap-6 bg-slate-50 dark:bg-gray-800 px-5 py-2.5 rounded-2xl border border-slate-200 dark:border-gray-750 shadow-inner ring-1 ring-white/10">
            <div className="flex items-center gap-2">
              <Clock className="h-4 w-4 text-brand-500" />
              <span className="text-sm font-medium text-slate-700 dark:text-gray-300">{currentTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
            </div>
            <div className="h-5 w-px bg-slate-200 dark:bg-gray-700" />
            <div className="flex items-center gap-2">
              <div className="h-6 w-6 bg-brand-100 dark:bg-brand-900/30 rounded-full flex items-center justify-center border border-brand-200 dark:border-brand-800/40 shadow-sm">
                <User className="h-3 w-3 text-brand-600 dark:text-brand-400" />
              </div>
              <span className="text-sm font-medium text-slate-700 dark:text-gray-300 capitalize">{user?.name}</span>
            </div>
          </div>
        </header>

        {/* Customer & Doctor Selection */}
        <div className="p-4 sm:p-6 shrink-0 bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 relative z-20">
        ...
        {/* Product Search */}
        <div className="p-4 sm:p-6 shrink-0 bg-slate-50 dark:bg-gray-950 border-b border-gray-100 dark:border-gray-800 shadow-sm relative z-10">
          <div className="relative max-w-3xl mx-auto">
            ...
            <input
              ...
              className={cn(
                'w-full pl-16 pr-6 py-5 rounded-2xl border-2 transition-all bg-white dark:bg-gray-900 text-gray-900 dark:text-white shadow-sm hover:shadow-md focus:bg-white dark:focus:bg-gray-900',
                ...
                (showResults && searchQuery.length >= 2) ? 'border-brand-500 rounded-b-none shadow-2xl relative z-50' : 'border-slate-200 dark:border-gray-700'
              )}
            />
            ...
            {showResults && searchQuery.length >= 2 && (
              <div
                ref={searchRef}
                className="absolute top-full left-0 right-0 z-50 bg-white dark:bg-gray-900 border-2 border-brand-500 border-t-0 rounded-b-xl shadow-xl max-h-[60vh] overflow-auto"
              >
                {loadingMedicines ? (
                  ...
                  searchResults.map((medicine) => (
                    <button
                      ...
                      className={cn(
                        "w-full flex items-center justify-between p-4 transition-colors border-b border-gray-100 dark:border-gray-800 last:border-0 text-left",
                        medicine.availableQty <= 0 ? "opacity-60 cursor-not-allowed bg-gray-50 dark:bg-gray-950" : "hover:bg-brand-50 dark:hover:bg-gray-800"
                      )}
                    >
                      <div>
                      ...
                      <div className="text-right">
                        ...
                        {medicine.availableQty > 0 ? (
                          <p className={cn(
                            'text-xs font-medium',
                            medicine.availableQty < 10 ? 'text-red-500' : 'text-gray-500 dark:text-gray-400'
                          )}>
        ...
        {/* Cart Panel */}
        <div className="flex-1 overflow-auto p-4 sm:p-6 bg-gray-50 dark:bg-gray-950/50">
          {cartItems.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-gray-400 animate-in fade-in duration-500">
              <div className="relative h-24 w-24 mb-6 group">
                ...
                <div className="relative h-full w-full bg-white dark:bg-gray-900 rounded-full flex items-center justify-center shadow-xl border border-gray-100 dark:border-gray-800 transition-transform duration-500 group-hover:scale-105">
                  <ShoppingCart className="h-10 w-10 text-brand-400" />
                </div>
              </div>
              <p className="text-xl font-medium text-gray-600 dark:text-gray-400">Cart is empty</p>
              <p className="text-sm mt-2">Search and add medicines to start billing</p>
            </div>
          ) : (
            <div className="space-y-4 max-w-4xl mx-auto">
              <div className="flex items-center justify-between mb-2 px-2">
                <h3 className="font-medium text-gray-700 dark:text-gray-300">Current Order ({cartItems.length} items)</h3>
                <button
              ...
              {cartItems.map((item, index) => (
                <div
                  ...
                  className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm p-5 hover:shadow-xl hover:-translate-y-0.5 hover:border-brand-200 transition-all duration-300 group relative overflow-hidden"
                >
                  {/* Decorative Side Bar */}
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      ...
                      <p className="text-xs font-medium text-gray-500 dark:text-gray-400 mt-1 flex items-center gap-2">
                        <span className="bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded text-gray-600 dark:text-gray-400">Batch: {item.batchNumber}</span>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between bg-gray-50 dark:bg-gray-950/80 rounded-xl p-3 sm:p-4 border border-gray-100 dark:border-gray-800/50 gap-4">
                    {/* Quantity Control */}
                    <div className="flex items-center gap-3 justify-between sm:justify-start w-full sm:w-auto">
                      <div className="flex items-center bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden shadow-sm">
                        <button
                          type="button"
                          onClick={() => updateQuantity(index, item.quantity - 1)}
                          className="p-2 hover:bg-gray-50 dark:bg-gray-950 text-gray-600 dark:text-gray-400 border-r border-gray-200 dark:border-gray-700 transition-colors"
                        >
                        ...
                        <input
                          ...
                          className="w-14 text-center py-2 focus:outline-none text-base font-medium text-brand-700 bg-white dark:bg-gray-900"
                          min="1"
                        ...
                        <button
                          type="button"
                          onClick={() => updateQuantity(index, item.quantity + 1)}
                          className="p-2 hover:bg-gray-50 dark:bg-gray-950 text-gray-600 dark:text-gray-400 border-l border-gray-200 dark:border-gray-700 transition-colors"
                        >
                    ...
                    {/* Item Discount & Total */}
                    <div className="flex items-center gap-4 sm:gap-6 w-full sm:w-auto justify-between sm:justify-end border-t sm:border-t-0 border-gray-200 dark:border-gray-700/60 pt-3 sm:pt-0">
                      <div className="flex flex-col items-start sm:items-end">
                        ...
                        <input
                          ...
                          className="w-16 px-2 py-1 rounded-md border border-gray-200 dark:border-gray-700 text-sm font-medium text-center focus:border-brand-500 focus:ring-1 focus:ring-brand-500 bg-white dark:bg-gray-900 text-gray-900 dark:text-white"
                          min="0"
                          max="100"
                        />
                      </div>
      ...
      <div className="w-full lg:w-96 bg-slate-50 dark:bg-gray-950 border-t lg:border-t-0 lg:border-l border-gray-200 dark:border-gray-700 flex flex-col lg:overflow-hidden shrink-0">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between gap-4">
                <span className="text-sm text-gray-500 dark:text-gray-400 whitespace-nowrap">Bill Discount</span>
                <div className="flex items-center gap-2">
                  <select
                    value={discountType}
                    onChange={(e) => setDiscountType(e.target.value)}
                    className="text-xs border border-gray-200 dark:border-gray-700 rounded px-1 py-1 bg-white dark:bg-gray-900 text-gray-900 dark:text-white"
                  >
                  ...
                  <input
                    ...
                    placeholder="0"
                    className="w-16 text-right text-sm border-b border-gray-200 dark:border-gray-700 focus:border-brand-500 outline-none bg-transparent text-gray-900 dark:text-white"
                    min="0"
                  />
                </div>
              </div>
                ...
                <button
                  ...
                  className={cn(
                    ...
                    paymentMethod === method.id
                      ...
                      : 'bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:border-brand-200'
                  )}
                >
