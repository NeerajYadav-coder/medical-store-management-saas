...
export function PageLoader({ text = 'Loading...' }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-white dark:bg-gray-900 dark:bg-gray-950">
      <div className="flex flex-col items-center gap-4">
        ...
        <p className="text-gray-600 dark:text-gray-400 dark:text-gray-300 font-medium">{text}</p>
      </div>
    </div>
  )
}
...
export function SkeletonCard({ className }) {
  return (
    <div className={cn('rounded-lg border border-gray-200 dark:border-gray-700 dark:border-gray-700 p-4 space-y-4', className)}>
  ...
  return (
    <div className={cn('space-y-2', className)}>
      {Array.from({ length: rows }).map((_, rowIndex) => (
        <div key={rowIndex} className="flex gap-4 p-4 border-b border-gray-100 dark:border-gray-800 dark:border-gray-800">
...
export function LoadingOverlay({ isLoading, text, children }) {
  ...
  return (
    <div className="relative">
      ...
      <div className="absolute inset-0 flex items-center justify-center bg-white dark:bg-gray-900/80 dark:bg-gray-900/80">
        <Loader text={text} />
      </div>
    </div>
  )
